...
tSpan = prb.guess.time;
tGuess = linspace(tSpan(1), tSpan(2), prb.nPts);
xGuess = interp1(tSpan',prb.guess.state',tGuess')';
uGuess = interp1(tSpan',prb.guess.control',tGuess')';
[qGuess,ind] = conCat(tGuess,xGuess,uGuess);

%construct lower bound vector
tLb = linspace(prb.bnd.tiLow, prb.bnd.tfLow, prb.nPts);
...
tUb = linspace(prb.bnd.tiUpp, prb.bnd.tfUpp, prb.nPts);
...
@(q)(objectiveFunc...   %use our objective func with the inputs:
...
[qSoln,objVal,flag,output] = fmincon(fPrb);

[soln.t,soln.x,soln.u] = unCat(qSoln,ind);
...
ind.x = [ind.t(end)+1 ind.t(end)+numel(x), m];
